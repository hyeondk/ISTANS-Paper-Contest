### Code : 대형차 Univariate Time Series Analysis(지역별)
### Writer : Donghyun Kim
### Date : 2020.09.17
...
autoplot(forecast(fit1, h = 12), ylab = "자동차 수",
         ylim = c(238000, 238000 + 18000),
         main = "인천 지역 예측 시계열그림(대형차 기준)")
...
autoplot(forecast(fit2, h = 12), ylab = "자동차 수",
         ylim = c(findvalue(yeosu)[1], findvalue(yeosu)[1] + 14000),
         main = "여수 지역 예측 시계열그림(대형차 기준)")
...
autoplot(forecast(fit3, h = 12), ylab = "자동차 수",
         ylim = c(234000, 234000 + 25000),
         main = "부산 지역 예측 시계열그림(대형차 기준)")
...
autoplot(forecast(fit4, h = 12), ylab = "자동차 수",
         ylim = c(findvalue(pyeongtaek)[1], findvalue(pyeongtaek)[1] + 14000),
         main = "평택 지역 예측 시계열그림(대형차 기준)")
...
autoplot(forecast(fit5, h = 12), ylab = "자동차 수",
         ylim = c(findvalue(jeju)[1], findvalue(jeju)[1] + 14000),
         main = "제주 지역 예측 시계열그림(대형차 기준)")
